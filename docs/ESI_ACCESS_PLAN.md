...
resolution_fallback`, `/api/admin/structures/fallback`). When on, a tenant
with no `structure_name_resolution` characters of its own can still
resolve a structure using the *Default Tenant's* own operator characters -
the click that turns the switch on is the informed consent for that
(decision 6b), not a default every fresh tenant gets. Admin's own bulk
resolution (`admin.do_resolve_structure_names`) does not depend on this
switch at all - it always uses the acting admin's own tenant's characters,
