...
def do_list_intake_sources() -> dict:
    sources = [
        ...
        for source_id, source_kind, owner_name, hangar_flag, label
...
def do_add_intake_source(source_kind: str, hangar_flag: str,
    ...
    if not owner_name or not owner_name.strip():
        raise ActionError("owner_name is required for an intake source.")
    owner_name = owner_name.strip()
    if label is not None:
        label = label.strip() or None
    source_id = storage.add_sorting_intake_source(
        source_kind, hangar_flag, owner_name=owner_name, label=label,
    )
    return {
        "id": source_id,
        "source_kind": source_kind,
...
def do_list_available_corps() -> dict:
    return {"corps": storage.load_tenant_corp_names()}
