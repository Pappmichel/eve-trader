"""Cross-tool actions - unlike actions.py/production/actions.py/doctrine/
actions.py (one per tool), this module deliberately spans all four
tenant-facing tools, same spirit as portfolio.py/scheduler.py (see
CLAUDE.md's "Two tools, one backend"). Still follows the `do_*` pattern
...
right now, and how much of it does each tool still want" - a read-only
aggregation across Trading/Production/Doctrine/Ore & Minerals, no writes, no
reservation/allocation logic (EVE itself has no such concept either - see
CLAUDE.md's "Nicht tun").
"""
...
def _production_wanted_by_type(cfg: ProductionConfig = PRODUCTION_CONFIG) -> dict[int, float]:
    """stock_targets' backup_stock (the internal-reserve Soll - see
    ...
    I still need to sort into my own hangar" question, not a "how much
    should I list for sale" one, so the extra market-target pools aren't
    relevant here."""
...
def _ore_minerals_wanted_by_type() -> dict[int, float]:
    ...
    (Ore & Minerals doesn't track its own ESI stock at all - see the parent
    task's own note), so the raw required_qty is the closest real read
    available."""
    return {type_id: qty for type_id, _name, qty in storage.load_mineral_requirements() if qty > 0}


def do_sorting_list(cfg: TradingConfig = TRADING_CONFIG, production_cfg: ProductionConfig = PRODUCTION_CONFIG,
                     doctrine_cfg: DoctrineConfig = DOCTRINE_CONFIG) -> dict:
    """Sorting-helper for the shared Wareneingang: for every type_id
    currently sitting in cfg.intake_hangar_flag (storage.assets_at_flag),
    how much is there and how much does each tool still want of it. Purely
    read-only aggregation - no reservation/allocation, EVE itself has no such
    concept (the player still decides by hand where each unit actually
    goes).

    Returns {"rows": [...]} where each row is
    {type_id, type_name, intake_qty, wanted_by_tool: [{tool, wanted_qty}],
    unclaimed}. `unclaimed` is True when no tool's own demand covers this
    type_id at all - still included (not filtered out), so a Wareneingang
    stack nobody currently wants shows up as a clear "nothing needs this
    right now" instead of silently vanishing from the list.

    Empty `rows` (not an error) when intake_hangar_flag is unset - the
    feature is simply off until an operator configures which hangar
    division the shared intake actually is."""
    flag = cfg.intake_hangar_flag
    if not flag:
        return {"rows": []}
    intake = storage.assets_at_flag(flag)
    if not intake:
        return {"rows": []}

    trading_wanted = _trading_wanted_by_type()
    production_wanted = _production_wanted_by_type(production_cfg)
    doctrine_wanted = _doctrine_wanted_by_type(doctrine_cfg)
    ore_minerals_wanted = _ore_minerals_wanted_by_type()

    rows = []
    for type_id, intake_qty in intake:
        wanted_by_tool = []
        for tool, wanted_map in (
            ("trading", trading_wanted), ("production", production_wanted),
