...
| `eve_trader/goonmetrics_client.py` | `eve_trader_local/goonmetrics_client.py` | The current-price fetch shape (`appraise.gnf.lt/market/{slug}/prices.json`, its price parsing and retry-with-backoff) and the caching *pattern*: module-level cache dicts keyed by market, with a per-market lock rather than one shared lock, so a multi-second Jita fetch never blocks a concurrent home-market fetch that shares no cache entry with it. Plus `ESIClient.structure_order_stats_bulk_or_goonmetrics` (in `esi_client.py`, same as here) — the failsafe that falls back to a Goonmetrics snapshot when no seller is logged in or the real structure order book fails, returning `(stats, used_fallback)` with volumes zeroed. | Ported 2026-09-02. Deliberately left out on the local side: `price_history`/`price_history_chunked`/`HistoryPoint`/`_parse_history_xml` and the ESI per-type history fallback — that's the *region price-history* source (a different endpoint answering a different question, see this file's own "Price sources matrix"), used only by `history_backtest.py`/`shortlist.py`/`production/engine.py`, all still candidates below; port it with whichever of those lands there, along with `goonmetrics_history_base`/`chunk_size`-equivalent config. Added on the local side: `TradingConfig.goonmetrics_appraise_base` (this repo hardcodes it as a module constant `APPRAISE_BASE`; eve-trader-local already has an `esi_base` config precedent, so it became a field there). |
...
ESI-client/Goonmetrics-client foundation has been built so far). When one
gets ported over as a real local feature, add it to the table above and keep
the following in sync going forward. Each entry names *what part* is
shareable — usually the computation, not the surrounding storage/config
wiring:
