...
def test_activate_shortlist_items(tenant):
    # GitHub issue #35: the reactivation counterpart to
    # deactivate_shortlist_items above.
    storage.upsert_shortlist([
        ShortlistItem(item="Stay Inactive", item_id=1, category="Material", volume_m3=1.0, active=False),
        ShortlistItem(item="Recovered", item_id=2, category="Material", volume_m3=1.0, active=False),
    ])

    storage.activate_shortlist_items([2])
...
def test_activate_shortlist_items_noop_on_empty_list(tenant):
    storage.upsert_shortlist([
        ShortlistItem(item="Stay Inactive", item_id=1, category="Material", volume_m3=1.0, active=False),
    ])
